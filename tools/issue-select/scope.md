...
My recent projects involve AI-assisted web applications and agent workflows, including document-processing products. I want a bounded first contribution that improves correctness and has a concrete regression test, rather than an open-ended redesign. Prefer document ingestion, LLM-output handling, or agent-tool bugs with explicit inputs and expected outputs. Keep the first issue small enough for the weekly course workflow; I am not claiming prior expertise in this repository or a specific number of available hours.
